...
public class GlobalErrorWebExceptionHandler extends AbstractErrorWebExceptionHandler {
    ...
    private Mono<Tuple2<ErrorDTO.ErrorDescription, HttpStatus>> buildErrorResponse(
        ...
        return Mono.just(ErrorDTO.ErrorDescription.builder()
                        .httpStatus(technicalException.getTechnicalErrorMessage().getCode())
                        .message(technicalException.getTechnicalErrorMessage().getDescription())
                        .build())
                .zipWith(getStatusCode(technicalException));
    }

    private Mono<Tuple2<ErrorDTO.ErrorDescription, HttpStatus>> buildErrorResponse(
            BusinessException businessException) {
        return Mono.just(ErrorDTO.ErrorDescription.builder()
                        .message(businessException.getBusinessErrorMessage().getReason())
                        .httpStatus(businessException.getBusinessErrorMessage().getCode())
                        .build())
    ...
    private Mono<Tuple2<ErrorDTO.ErrorDescription, HttpStatus>> buildErrorResponse(Throwable throwable) {
        ...
        return Mono.just(ErrorDTO.ErrorDescription.builder()
                        .httpStatus((reason.contains("404 NOT_FOUND")) ? TechnicalErrorMessage.RESOURCE_NOT_FOUND.getCode() :
                                TechnicalErrorMessage.UNEXPECTED_EXCEPTION.getCode())
                        .message((reason.contains("404 NOT_FOUND")) ? TechnicalErrorMessage.RESOURCE_NOT_FOUND.getDescription() :
                                TechnicalErrorMessage.UNEXPECTED_EXCEPTION.getDescription())
                        .build())
    ...
    private String buildMessageDefault(Throwable throwable) {
        return Optional.ofNullable(throwable.getMessage())
                .orElse(TechnicalErrorMessage.UNEXPECTED_EXCEPTION.getDescription());

    }


    public static Mono<HttpStatus> getStatusCode(TechnicalException technicalException) {
        return Mono.just(HttpStatus.valueOf(technicalException.getTechnicalErrorMessage().getCode()));
    }

    public static Mono<HttpStatus> getStatusCode(BusinessException businessException) {
        return Mono.just(HttpStatus.valueOf(businessException.getBusinessErrorMessage().getCode()));
    }
